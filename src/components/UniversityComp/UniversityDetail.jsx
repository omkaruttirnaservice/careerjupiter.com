...
const UniversityDetail = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-2xl font-semibold text-gray-600">Loading university details...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-2xl font-semibold text-red-600">Error: {error.message}</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-10 mt-5">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col items-center justify-center">
          <h1 className="text-5xl font-bold text-white">{uni.universityName || 'Unknown University'}</h1>
          <p className="text-lg text-white mt-2">Category: {uni.Category || 'N/A'}</p>
        </div>
      </div>
      ...
      <div className="container mx-auto mt-8 px-4">
        <div className="bg-white shadow-md rounded-lg p-6">
          ...
          <div className="flex items-center mb-8">
            ...
            <p className="text-lg text-gray-700">{uni.contactDetails?.phoneNumber || 'No contact available'}</p>
          </div>
